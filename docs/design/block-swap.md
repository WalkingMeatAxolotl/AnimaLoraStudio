...
- 一句话结论：**训练口径估算开销约 9–10%，换 22.6GB 显存，可把 K2 下限从 32GB 拉到
  24GB 以下且不付精度代价；硬件损耗担忧经 1.4TB 传输实测排除（PCIe replay 增量 0）。**
- 日期：2026-07-21
...
**训练口径的完整成本估算**（1024²，全 28 层 swap，纯计算基线 28 × 103.7ms = 2.90s）：

| 成分 | 每步 | 占比 |
|---|---|---|
| 前向段暴露（1.1ms × 28） | 31 ms | 1.1% |
| HBM 争用（4.4ms × 28 × 2 个传输窗口） | 246 ms | 8.5% |
| **合计** | **277 ms** | **≈ 9.5%** |

即：**训练口径实际开销约 9–10%，不是暴露项单独看的 1%**。这个数字仍未端到端实测
（§5.3-1），但比只算暴露的 1% 保守可信得多。
...
| blocks_to_swap | 常驻显存 | 估算训练开销 | 意义 |
|---|---|---|---|
| 0（现状） | 25.8 GB | 0% | 32GB 余量≈0（Phase 0 实测） |
| 14 | 14.5 GB | ≈ 4.8% | 32GB 余量充裕；**24GB 可行** |
| 28 | 3.2 GB | ≈ 9.5% | **16GB 理论可行** |
...
**唯一不对称、需要在设计时就处理好的点**：Krea2 的循环在 `modeling/` 内部（对
ComfyUI 有逐字 parity 要求，改动要克制），Anima 的在训练侧 `forward.py`（自由）。
组件必须能从**外部**包住 ModuleList 而不要求改模型内部代码，否则 Anima 那侧会被迫
复制一份逻辑 —— 那就是 `02-ecosystem-survey.md` §7 批评 SimpleTuner 的按族扇出。

## 8. 剩余开放问题

1. **Gate-0 门槛（B0/B1）**：两个口径的百分比含义不同，建议分开定 ——
   - **训练口径** ≈ 9.5%（§5.1 估算）：换 22.6GB 显存。建议门槛 **15%**。
   - **推理口径** = 11–19%（实测）：出图时间的直接减速。建议门槛 **25%**（判据是
     「慢 1/4 但能出」vs「完全不能出」，比训练宽）。
   - 待用户确认这两个数字。
2. **下一步是先补训练口径端到端实测（§5.3-1）还是直接进实现**：估算的 9.5% 与只算
   暴露的 1% 差一个量级，建议先实测再动实现。
3. **pinned 分配失败的降级语义（B6）**：见 §8.1 的失败时机分析，据此定语义。
4. **HBM 争用项要不要继续压（B8）**：见 §8.2 判定，建议**不压**。
