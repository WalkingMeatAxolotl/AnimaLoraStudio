...
6. **打标加「范围」**。打标历来只扫 `train/`（`tag_worker` 写死），手动加入的验证图原本拿不到 caption，而 eval 用每张验证图的 caption 当生成 prompt。给 tag job 加 `scope`：`all`（默认 = train 全部 + validation）/ `validation` / 单个 train 文件夹名；worker 按 scope 收图，caption 原地写回（验证图落 `validation/<folder>/`）。验证集的 caption 应与 train 用同一 tagger / 设置打，eval 的 prompt 分布才贴近训练分布。scope 取文件夹名会拼进路径，endpoint 用 Kohya 文件夹规则挡 traversal。

## Addendum: 动漫域指标 + baseline·Δ + 删 inline 评估（2026-06-26，PR #324）

本次在 eval 系统上加动漫域指标 + baseline 对照，并**推翻 §8 的 inline 评估设计**。

1. **指标门控框架**。`eval_registry` 作为指标单一真相：每个指标声明 key / runner / 依赖模型 / 默认开关。「runner」= 一个评估 job 产 ≥1 指标（clip→clip_t+clip_i、dino→dino_i、ccip→ccip_i、tag→tag_recall）。全局 Settings 用 `enabled_metrics`（复选框）门控，`eval_auto` 两处 dispatch 只迭代「已启用的 runner」。新增指标 = 注册一个 runner + worker + 6 处 job-kind 登记。

2. **动漫域指标（默认关，按需勾）**。
   - **CCIP-I**（`deepghs/ccip_onnx`）：DINO-I 的动漫域替代，判「是否同一动漫角色」。纯 onnxruntime（384px / CLIP mean-std / feat 768d → metric head 出 NxN diff → `metrics.json` threshold 判同异），**不引 imgutils**（其 import 期自动 `pip install onnxruntime` 撞项目 build 策略）；按文件名选择性下载变体 3 文件，缺失时懒加载。仅单角色的角色 LoRA 有意义，且对角色一旦能还原就饱和到 1（粗判定）。
   - **tag_recall**：CLIP-T 的动漫原生替代。复用 WD14 对生成图回标，`recall = |生成图 tag ∩ prompt tag 且在 WD14 词表内| / |词表内 prompt tag|`。零新模型，仅 booru-tag caption 有意义。

3. **baseline + Δ**。训练后评估额外出一组**纯底模**（`lora_scale=0`、同 prompt/seed）对照，每 task 一次（`_ensure_baseline_queued` 去重）；`_attach_baseline_delta` 给各 checkpoint 挂 `delta = 指标 − baseline`。前端：每张指标图画 baseline 水平参考线（曲线在它上/下方=优/劣于底模）、表格每行带 Δ。注意 `lora_scale` 解析不能写 `x or 1.0`——0.0 是 falsy 会回退成 1.0 使 baseline 形同正常 LoRA、Δ 恒 0。

4. **删除 inline 评估 + checkpoint-trigger 时机（推翻 §8 + §3 的 `auto_eval_trigger`）**。inline 在训练热循环里复用 `ctx.model`（卸/装 LoRA 有风险）、与训练抢显存、且需大量兼容代码。全部删除：`runtime/training/eval_auto.py`、loop 的 `_handle_eval_checkpoint` 钩子、finalize 的 `eval_checkpoint_saved` emit、studio 的 `queue_checkpoint_eval` / `run_checkpoint_eval_for_task`、supervisor 的 `eval_checkpoint_saved` handler、`secrets.auto_eval_trigger` 字段 + 前端时机下拉。**评估统一走训练后的独立 job**（GPU-bound 被 supervisor 自动延后到训练释放显存才跑，零争用、与训练解耦）。是否评估仍由 per-version `eval_validation_enabled` 门控。

5. **重跑数据模型：永远只显示当次**。每次「运行评估」先**自动清空上一轮**（`_clear_previous_task_eval` 删旧 run 文件 + 取消未完成的旧 job；**已完成的旧 job 不改状态**，不污染历史）。`/eval/jobs` 只返回 **run 仍存在** 的 job —— 旧 run 删了其 job 自然不再进合并日志。不做跨轮 merge / dedup（旧值无参考性），不设手动「清空」按钮。
