...
> PR #138 只落地 eval manifest 协议/API。后续生产化工作按小 PR 拆分；当前 PR1 增加 sample runner 与 metric result contract，但仍不引入具体指标模型、Settings/Monitor UI 或训练自动触发。
...
| 2（当前 PR1） | Eval sample runner：按 manifest 对 checkpoint 出图，保存单图与 `run.json` metadata | 不接 CLIP / DINO / CMMD | 0 + 1 |
| 3（当前 PR1） | Metric result schema：定义 `metrics.json`、embedding cache 目录、API 返回格式、空状态 | 不实现具体指标 | 2 |
