...
> 状态：已部分落地（PR #324）。本文档保留作调研记录（现状、三个优化方向、社区/论文/
> 二轮交叉验证）。**已采纳并实现**：指标门控框架、tag_recall、CCIP-I、baseline+Δ —— 详见
> [ADR 0011](../adr/0011-lora-eval-metrics.md) 的「动漫域指标 + baseline·Δ + 删 inline
> 评估」Addendum。**仍未实现**：记忆护栏（§盲区①）、多样性塌缩（§盲区②）、wd_i / 质量门。
>
> 注：本文档描述的「一个 metric runner = 一个评估作业」执行模型已被 issue #465 推翻 ——
> 一次评估现在是一个 `eval_session` 作业，`1+M` 个阶段跑在它内部（见 ADR 0011 的
> 「EvalSession」Addendum）。**指标算法本身不受影响**，本文档的指标选型结论仍然成立；
> 只有「怎么排队执行」那部分过时了。
>
> 相关：[ADR 0011](../adr/0011-lora-eval-metrics.md)（eval 指标体系）、PR #138 /
> #319（held-out 验证集）/ #323（评估可见性）/ #324（本文档采纳项）/ #465（EvalSession）。
