...
「队列」页查看任务，进**任务详情**看日志 / 监控 / 输出（含一键全量 zip 下载）。若详情加载失败，可在错误提示中点**重新加载**，只重新读取当前任务，不会重新运行任务；刷新失败时已显示的详情会保留。数据任务列表加载失败时也可在错误提示中重新加载；刷新失败保留已有任务行，不会将失败显示为“暂无数据任务”。GPU 队列默认只看训练任务，可展开筛选切换类型；进行中/待办与历史分别报告读取错误，可只重新加载失败的部分。空态说明当前任务类型或筛选范围。任务的“结束”时间不代表成功，等待状态不显示推算排位；无有效步数时显示未知进度。时间按界面语言显示，列表省略文本可悬停查看，详情参数保留全文。
