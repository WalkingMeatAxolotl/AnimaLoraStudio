...
独立的 **训练监控** 是跨项目、只读的训练驾驶舱：默认打开当前运行中的训练，也可在顶部切换
有监控记录的历史训练。它不负责暂停、取消或重试；这些操作仍在 **队列 → 任务详情** 中完成。
页面右上角的 **查看任务详情** 会打开当前任务，底部日志抽屉则直接显示同一任务的 `run.log`。
书签中的 `?task=<任务 id>` 可以打开列表之外的旧训练；不存在或不是训练任务的链接会明确报错，
不会伪装成空监控。

页面把监控证据区分为 **实时**、**重连中**、**已同步** 和 **历史快照**。HTTP 快照读取成功
只代表数据已同步；只有 SSE 长连接已打开才显示“实时”。连接或刷新失败时，最近一次成功的数据
仍会保留并显示更新时间，可用 **重新读取** 局部重试，不会重新运行训练。首次读取失败、训练尚未
写入第一份状态、历史任务没有监控文件也会显示不同的说明和恢复方向。

监控面板沿用紧凑的训练工作台布局：
- **顶部状态行**：实时证据、最后更新时间和 step 进度，保持为轻量单行反馈
- **六项指标带**：step、近期 loss、全程平均 loss、实际学习率（含优化器 `d`）、显存和 ETA；
  ETA 卡同时显示已用时间与迭代速度，未上报的字段保持固定位置并显示“未上报”
- **左侧训练样图**：缩略图时间线和可放大的当前样图；聚焦缩略图后可用方向键、Home、End 切换
- **右侧完整趋势图**：loss、学习率与优化器 `d` 纵向排列并始终直接显示，各自可调 EMA 平滑；
  读屏会获得数据点数量、step 范围、当前值和值域摘要

页面保持样图与趋势图的固定工作区关系，间距和文字会跟随设置中的紧凑 / 默认 / 宽松密度。
