...
监控面板显示：
- **进度与时间**：step、epoch、已用时间和预计剩余时间
- **优化信号**：近期 loss、全程平均 loss、实际学习率和优化器 `d`
- **运行资源**：迭代速度与显存占用；未上报的字段保持固定位置并显示“未上报”
- **训练样图**：缩略图时间线和可放大的当前样图；聚焦缩略图后可用方向键、Home、End 切换
- **完整趋势图**：loss、学习率与优化器 `d` 始终直接显示，各自可调 EMA 平滑；读屏会获得
  数据点数量、step 范围、当前值和值域摘要

窗口宽度不超过 1280px 时，样图与趋势图会改为上下排列，但不会隐藏高级指标或改变任务操作。
页面间距和文字会跟随设置中的紧凑 / 默认 / 宽松密度。
