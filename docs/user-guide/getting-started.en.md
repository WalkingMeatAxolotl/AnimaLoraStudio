...
View tasks on the **Queue** page; open **task detail** for logs / monitoring / output (with one-click full zip download). If details fail to load, use **Reload** in the error notice to fetch the current task again without rerunning it. A failed refresh keeps previously loaded details visible.
