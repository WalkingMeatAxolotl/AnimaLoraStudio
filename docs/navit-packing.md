...
navit_native_resolution: false # 单图按原生分辨率定尺寸（16px 对齐，不 resize/crop）
navit_multiscale: false        # 多尺度阶梯：为大图追加低 token 档缩小副本（需 navit_native_resolution）
navit_multiscale_token_ladder: "4096"  # 副本 token 档（逗号分隔）
navit_multiscale_loss_weight: 1.0   # 副本 loss 权重（1.0=等权）
```
...
`navit_token_budget > 0`、`navit_multiscale → navit_native_resolution`。
...
| 原生定尺寸 + 多尺度 | 同上 — `plan_native_fit_image` / `plan_multiscale_copy` |
...
- `test_navit_packed_objective`：训练步前向 + loss + 反向梯度有限；grad checkpoint 等价；ms_loss_weight 生效。
