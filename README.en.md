...
- **GPU**: NVIDIA (AMD / Apple Silicon not supported), per family:
  - **Anima**: **16 GB+ VRAM recommended** (RTX 4060Ti 16G / 4070Ti / 4080 / 3090 / 4090 / 5090, etc.); 8 GB works without block swap (turn off sample output + reduce batch / resolution). With **block swap** (all blocks swapped out), **training** reaches **6 GB-class GPUs** at 1024² with samples on (training-step peak about 1.9 GB of allocations plus the resident text encoder; about 11% slower), and **generation**'s DiT residency drops to about 0.6 GB.
