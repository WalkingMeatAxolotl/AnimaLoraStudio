...
def test_create_session_surfaces_captured_stderr_on_silent_downgrade(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """ORT C 层 dlopen 错（典型 `libcublasLt.so.12: cannot open ...`）直写 fd 2，
    `try/except str(exc)` 抓不到；静默降级路径必须从 captured_fd_stderr
    拿原始报错塞进 cuda_load_error，否则用户看到的只有「降级了」+ 没原因。"""
    fake_stderr = b"\x1b[31m[E:onnxruntime] libcublasLt.so.12: cannot open shared object file\x1b[0m\n"

    @contextlib.contextmanager
    def fake_capture():
        yield [fake_stderr]

    monkeypatch.setattr(onnx_tagger_base, "captured_fd_stderr", fake_capture)
    ...
    try:
        ...
        # 静默降级提示仍在
        assert "静默降级" in err
        # 关键：C 层 dlopen 报错被转发出来（去 ANSI 后的纯文本）
        assert "libcublasLt.so.12" in err
        # ANSI 色码已剥
...
def test_decode_captured_strips_ansi_and_nul() -> None:
    """_decode_captured 必须剥 ANSI 色码 + NUL（Windows ORT 写 fd 2 带 NUL）。"""
    captured = [b"\x1b[31mError\x1b[0m\x00 libcurand.so.10: not found"]
    out = onnx_tagger_base._decode_captured(captured)
    assert "Error" in out
    assert "libcurand.so.10" in out
    assert "\x1b" not in out
    assert "\x00" not in out


def test_decode_captured_empty_returns_empty_string() -> None:
    assert onnx_tagger_base._decode_captured([]) == ""
    assert onnx_tagger_base._decode_captured([b""]) == ""
