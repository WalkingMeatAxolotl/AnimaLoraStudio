...
from tools.benchmark_lycoris import (
    REPO, eager_backend, kernel_events_metric, memory_metrics, metadata,
    metric, synchronize, validate_config, write_json,
)
...
def isolated_environment(work: Path) -> dict[str, str]:
    ...
    env.update({"HF_HUB_OFFLINE": "1", "TRANSFORMERS_OFFLINE": "1", "HF_DATASETS_OFFLINE": "1",
                ...
                "WANDB_MODE": "disabled", "WANDB_DISABLED": "true", "LYCORIS_KERNEL_BACKEND": "torch",
                "PYTHONDONTWRITEBYTECODE": "1", "PYTHONUTF8": "1", "TOKENIZERS_PARALLELISM": "false"})
    return env


def deny_network_and_processes(event, args) -> None:
    """Worker-local audit hook: no downloads, external logging or installer processes.

    Python audit hooks are defense in depth, not a boundary against native code.
    Installed only in disposable training children, after environment metadata.
    """
...
def make_training_args(config: dict, bindings: dict, work: Path, algorithm: str):
    ...
    if algorithm not in {"lora", "lokr"}:
        raise ValueError("R1 training only supports LoRA/LoKr")
    ...
    values.update({"config": None, "model_family": "anima", "data_dir": str(work / "dataset"),
                   ...
                   "lokr_factor": config["factor"], "seed": config["seed"],
                   ...
                   "repeats": 1, "reg_repeats": 1, "lora_dropout": 0.0,
                   "log_every": 1})
...
class BenchmarkObserver:
    ...
    def loop_started(self, ctx):
        ...
        for key in ("batch_size", "grad_accum", "resolution", "mixed_precision", "attention_backend",
                    "grad_checkpoint", "cache_latents", "blocks_to_swap", "lora_rank", "lora_alpha",
                    "lokr_factor", "optimizer_type", "lr_scheduler", "max_steps"):
        ...
        if self.mode == "kernel" and (torch.device(self.device).type != "cuda" or
    ...
    def forward_started(self, ctx, *, batch_size):
        ...
        if self.mode == "fb" and active:
            self.sync(self.device)
            self.fb_start = self.clock()
        if self.mode == "kernel" and active and self.kernel_reason is None:
            import torch
    ...
    def result(self) -> dict:
        ...
        throughput = self.config["measured"] / wall if complete and wall and self.mode == "throughput" else None
        reason = None if throughput is not None else ("diagnostic_pass" if self.mode != "throughput" else "incomplete_or_invalid_training")
        counts = ({k: self.window_end_counts[k] - v for k, v in self.window_counts.items()}
                  if self.window_end_counts is not None else None)
        return {"schema_version": 1, "status": "complete" if complete else "incomplete", "kind": "synthetic_data_training",
                ...
                "kernel_unavailable": metric(None, "ms", "torch.profiler", "F/B diagnostic",
                    self.kernel_reason or ("separate_pass" if self.mode != "kernel" else "see_per_microbatch_metrics")),
...
def training_worker(config: dict, options) -> dict:
    ...
    result.update({"environment": env_info, "dataset": dataset,
                   "requested": {"algorithm": options.algorithm, "rank": config["rank"],
                                 "alpha": config["alpha"], "factor": config["factor"], "seed": config["seed"]}})
    return result
