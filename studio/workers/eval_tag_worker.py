"""Eval tag-recall worker.

Runs WD14 tag-recall for one completed eval sample run and writes metrics.json.
"""
...
from studio.services import eval_tag
...
def run(job_id: int) -> int:
    ...
    try:
        ...
        progress(f"[start] eval_tag run={run_id} version={version['label']}")
        ...
        state = (result.get("metric_states") or {}).get("tag_recall") or {}
        progress(f"[done] tag_recall={state.get('status')} value={state.get('value')}")
        return 0
    except Exception as exc:  # noqa: BLE001
        logger.exception("eval tag worker crashed (job_id=%s)", job_id)
