...
Runs DINO-I for one completed eval sample run and writes metrics.json.
"""
...
from studio.services import eval_dino
...
def run(job_id: int) -> int:
    ...
    if not job:
        ...
        return 1
    if job["kind"] != eval_dino.JOB_KIND:
    ...
    try:
        ...
        model_name = str(params.get("model_name") or eval_dino.DEFAULT_MODEL_NAME)
        ...
        progress(f"[start] eval_dino run={run_id} version={version['label']}")
        ...
        dino_i = (result.get("metric_states") or {}).get("dino_i") or {}
        progress(f"[done] dino_i={dino_i.get('status')} value={dino_i.get('value')}")
        return 0
    except Exception as exc:  # noqa: BLE001
        logger.exception("eval dino worker crashed (job_id=%s)", job_id)
