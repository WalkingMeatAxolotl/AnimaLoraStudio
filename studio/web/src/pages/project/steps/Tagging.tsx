...
export default function TaggingPage() {
  ...
  // CLTagger 变体列表（model dropdown 选项）；拉不到 → dropdown 退化为只显示当前值。
  const [clVariants, setClVariants] = useState<CLTaggerVariantInfo[]>([])

  useEffect(() => {
    void api
      .getModelsCatalog()
      .then((c) => setClVariants(c.cltagger.variants))
      .catch(() => {})
  }, [])
